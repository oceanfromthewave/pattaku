...
router.post("/rooms", auth, async (req, res) => {
  try {
    ...
    if (!name || !name.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "채팅방 이름을 입력해주세요." });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "채팅방 생성에 실패했습니다." });
  }
});
...
router.get("/my-rooms", auth, async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "참여 중인 채팅방 목록을 불러오는데 실패했습니다.",
      });
  }
});
...
router.post("/rooms/:roomId/join", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "채팅방 참여에 실패했습니다." });
  }
});
...
router.post("/rooms/:roomId/leave", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "채팅방 나가기에 실패했습니다." });
  }
});
...
router.get("/rooms/:roomId/messages", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "메시지를 불러오는데 실패했습니다." });
  }
});
...
router.post("/direct/:targetUserId", auth, async (req, res) => {
  try {
    ...
    if (parseInt(targetUserId) === userId) {
      return res
        .status(400)
        .json({ success: false, message: "자기 자신과는 채팅할 수 없습니다." });
    }

    const directMessage = await chatRoomModel.getOrCreateDirectMessageAsync(
      userId,
    ...
    res
      .status(500)
      .json({ success: false, message: "1:1 채팅방 생성에 실패했습니다." });
  }
});
...
router.get("/unread-count", auth, async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "안읽은 메시지 수를 조회하는데 실패했습니다.",
      });
  }
});
...
router.get("/rooms/type/:type", auth, async (req, res) => {
  try {
    ...
    if (!validTypes.includes(type)) {
      return res
        .status(400)
        .json({ success: false, message: "유효하지 않은 채팅방 타입입니다." });
    }
