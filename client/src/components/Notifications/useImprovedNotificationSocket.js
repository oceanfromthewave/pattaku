...
export default function useImprovedNotificationSocket(userId) {
  ...
  const loadInitialNotifications = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `✅ 초기 알림 로드 완료: ${mapped.length}개 (읽지않음: ${
  ...
  const attemptReconnect = useCallback(() => {
    if (reconnectAttemptsRef.current >= MAX_RECONNECT_ATTEMPTS) {
      console.error(
        `❌ WebSocket 재연결 포기: 최대 시도 횟수(${MAX_RECONNECT_ATTEMPTS}) 초과`
      );
      setIsReconnecting(false);
      toast.error(
        "실시간 알림 연결에 실패했습니다. 페이지를 새로고침해주세요.",
        {
    ...
    console.log(
      `🔄 WebSocket 재연결 시도 ${reconnectAttemptsRef.current}/${MAX_RECONNECT_ATTEMPTS}`
    );
  ...
  const connectWebSocket = useCallback(() => {
    ...
    try {
      ...
      ws.onclose = (event) => {
        setIsConnected(false);
        console.warn(
          `🔌 WebSocket 연결 종료: ${userId}, 코드: ${event.code}, 이유: ${event.reason}`
        );
